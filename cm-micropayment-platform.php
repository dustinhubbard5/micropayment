...
//		add_action( 'init', array( get_class(), 'periodicWalletChanges' ) );
		...
		require_once CMMP_PLUGIN_DIR . '/shared/cm-micropayment-platform-const.php';
		require_once CMMP_PLUGIN_DIR . '/shared/functions.php';
		require_once CMMP_PLUGIN_DIR . '/shared/models/label.php';
		require_once CMMP_PLUGIN_DIR . '/shared/classes/cm-micropayment-mu.php';
		require_once CMMP_PLUGIN_DIR . '/shared/classes/cm-micropayment-woo.php';
		require_once CMMP_PLUGIN_DIR . '/shared/classes/cm-micropayment-woo-gateway.php';
		require_once CMMP_PLUGIN_DIR . '/shared/classes/CmDokan.php';
		require_once CMMP_PLUGIN_DIR . '/shared/classes/cm-micropayment-woo-generate-discount.php';
		require_once CMMP_PLUGIN_DIR . '/shared/classes/cm-micropayment-edd-gateway.php';
		require_once CMMP_PLUGIN_DIR . '/shared/classes/cm-micropayment-edd-grant-points.php';
		require_once CMMP_PLUGIN_DIR . '/shared/classes/cm-micropayment-edd-generate-discount.php';
		require_once CMMP_PLUGIN_DIR . '/shared/classes/cm-micropayment-edd-paypal-payout.php';
		require_once CMMP_PLUGIN_DIR . '/shared/classes/cm-micropayment-edd-stripe-payout.php';
		require_once CMMP_PLUGIN_DIR . '/frontend/classes/cm-micropayment-platform-frontend-wallet.php';
		...
		if ( is_admin() ) {
			 ...
			 * Backend
			 */
			require_once CMMP_PLUGIN_DIR . '/backend/cm-micropayment-platform-backend.php';
		...
		if ( $cmmp_isLicenseOk ) {
			 ...
			 * Frontend
			 */
			require_once CMMP_PLUGIN_DIR . '/frontend/cm-micropayment-platform-frontend.php';
	...
	public static function getExternalsWallets() {
		...
		$response = wp_remote_get( "$url/wp-json/cmmpexapi/v1/get_wallets?key=" . $wallet_key );
	...
	public static function withdrawExternalWallet( $args ) {
		...
		$response     = wp_remote_get( "$url/wp-json/cmmpexapi/v1/withdraw_wallet_points_external?" .
		                               "key=$wallet_key&wallet_id={$args['wallet_id']}&type={$args['type']}&points={$args['points']}" );
	...
	public static function _activate() {
		...
		$sql = "CREATE TABLE " . $tablePrefix . "_defined_points_cost (
                ...
                points_value DECIMAL(10,1) NOT NULL,
		...
		if ( ! cmmicropaymentplatform::get_option( 'cm_micropayment_email_transfer' ) ) {
			$message = 'Transferred [amountPoints] points from [withdrawedWalletID] to [senderWalletID]';
	...
	static function runScheduled() {
		...
		try {
			...
			foreach ( $currentWallets as $wallet ) {
				CMMicropaymentPlatformWallet::syncExternalsWallet( [ 'wallet_name' => $wallet->wallet_name ] );
			}
		} catch ( \Exception $e ) {
	...
	public static function onUserCreated( $user_id ) {
		include_once CMMP_PLUGIN_DIR . '/frontend/classes/cm-micropayment-platform-frontend-wallet.php';
	...
	public function registerAjaxFunctions() {
		...
		add_action( 'wp_ajax_cmmicropayment_success', array(
			'CMMicropaymentPlatformFrontend',
			'finalizeTransaction'
		) );
		...
		add_action( 'wp_ajax_nopriv_create_wallet_id', array(
			'CMMicropaymentPlatformFrontend',
			'ajaxCreateWalletID'
		) );
	...
	public static function createWalletForExistingUsers() {
		...
		// copy remote wallets if they exits
		if ( CMMicropaymentPlatform::get_option( "cmmp-external-wallet" ) == 'on' ) {
			$remoteWallets = self::getExternalsWallets();
			if ( $remoteWallets ) {
				foreach ( $users as $u ) {
					if ( ! isset( $wallets[ $u->ID ] ) ) {
						$keyNumber = array_search( $u->user_email, array_column( $remoteWallets, 'user_email' ) );
						if ( $keyNumber !== false ) {
							$model->createWalletFromRemote( $u->ID, $remoteWallets[ $keyNumber ] );
						}
					}
				}
			}
	 ...
	 * @return string
	 */
	public static function getWalletByCode( $code ) {
		if ( empty( $code ) ) {
			return null;
		}

		$model  = new CMMicropaymentPlatformWallet();
		$wallet = $model->getWalletByCode( $code );
	...
	public static function periodicWalletChanges() {
		...
		if ( $wallets ) {
			foreach ( $wallets as $oneWalet ) {
				$walletObject->chargeWallet( $oneWalet->wallet_id, $amount, false, true, $comment );
			}
		}
		wp_clear_scheduled_hook( 'periodicWalletChanges' );
	...
	public static function convertType( $var ) {
		...
		if ( cmmicropaymentplatform::get_option( 'cm_micropayment_rounds_decimal', '' ) == 'lowest' ) {
